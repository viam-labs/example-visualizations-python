...
| `colorful_sphere.ply` | ASCII PLY        | ~178 KB | 4-level subdivision icosphere (2562 vertices / 5120 triangles, smoothly-shaded sphere) with per-vertex RGB colors derived from spherical coordinates — hue cycles around the longitude axis. Tests whether the viewer honors PLY's standard `property uchar red/green/blue` per-vertex color attributes. The item that uses this asset omits `color` from its config so embedded vertex colors aren't overridden by a uniform metadata tint. |
