...
| `bunny.stl`        | Binary STL         | ~15 KB  | Stanford bunny, 308 triangles after decimation. Centered on its X/Y centroid with bottom Z at 0 so it sits on the floor. Source: github.com/ecell/ecell4_docs (ASCII STL); converted to binary + meter-scale at build time. The Stanford bunny dataset is from the Stanford Computer Graphics Laboratory. |
