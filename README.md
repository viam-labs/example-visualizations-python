...
- `geometry_morph` — four-slot row demonstrating animation patterns that go beyond pose: pulsing sphere (radius), stretching box (single-axis dimension via the new `pulse axis` param), breathing capsule (smooth opacity oscillation), and a 5×5 sphere grid with phase-staggered `flicker` so the row reads as a wave of appearing-and-disappearing dots. Uses the new `breathe` and `flicker` animation modes.
...
| `flicker`    | `period_s` (3), `duty_cycle` (0.5), `phase_offset_s` (0) | Square-wave on/off via opacity 0 ↔ 1. `duty_cycle` is the fraction of each period the item is visible. `phase_offset_s` lets a row of items with the same period flicker out-of-phase, producing a wave/marquee effect across a grid. |
